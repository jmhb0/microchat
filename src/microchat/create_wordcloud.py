...
@click.command()
@click.option(
    "--input-file", type=click.Path(dir_okay=False, path_type=Path)
)
@click.option(
...
@click.option(
    "--filter_col", default=None, type=dict, help="Filter to apply to the dataframe. {column_name: value}"
)
...
@click.option("--random-seed", default=8675309, type=int, help="Random seed for reproducibility.")
...
def main(
    ...
    # instantiate
    wordcloud = WordCloud(width=width, height=height, background_color="white", random_state=random_seed)
    ...
    if output_file.exists():
        click.confirm(f"Output file {output_file.name} already exists! Do you want to overwrite?", abort=True)
