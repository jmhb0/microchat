...
def gen_choices(key_form, key_question_gen, key_choices_gen, seed=0):
    ...
    df_questions['choices'] = make_choices(df_questions['llm_response_choices'],
                                              seed)
...
def gen_choices_simple(df_questions, key_choices_gen, seed):
    ...
    for idx, row in df_questions.iterrows():
        prompt = prompt_template_simple[key_choices_gen]
        prompt = prompt.replace("{{question}}", row['question'])
        prompt = prompt.replace("{{sample_incorrect_answer}}", str(row['incorrect_answer']))
